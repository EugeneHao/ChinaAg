...
# (updated: we need the past year acreage as one of the covariates)
# acredat <- acredat %>% filter(year != 2000) 

# compute share (updated: not need )
...
# other include wheat 
multidat <- left_join(costdat, acredat %>% mutate(other = total - soybean - corn - rice) %>% 
                        gather(key = crop, value = acre, -region, -year, -total, -other) %>% 
                        mutate(crop = factor(crop, labels = c("corn", "rice", "soybean", "wheat"))))

pastacredat <- acredat
pastacredat$year <- pastacredat$year + 1

multidat <- left_join(multidat, pastacredat %>% gather(key = crop, value = acre, -region, -year, -total) %>% 
                        mutate(acre_lag = acre) %>% select(region, year, crop, acre_lag) %>% 
                        mutate(crop = factor(crop, labels = c("corn", "rice", "soybean", "wheat"))))
...
# use log of acreage as response variable 
multidat$acre[multidat$acre < 1] <- 1             # note here! 
regdat <- multidat %>% mutate(y = log(acre))
...
saveRDS(regdat, "From Hao/data/dongbei/regdat_V3.rds")
saveRDS(preddat, "From Hao/data/dongbei/preddat_V3.rds")
